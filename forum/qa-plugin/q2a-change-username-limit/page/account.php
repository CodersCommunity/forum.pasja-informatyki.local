...
function checkLastChangeUsername($userId): ?int
{
    $user = qa_db_read_one_assoc(
        qa_db_query_sub('
        SELECT userid, handle, username_change_date
        FROM ^users 
        WHERE userid=$',
    ...
    if (!empty($user) && isset($user['username_change_date'])) {
        $now = new DateTime('now');
        $lastChangeUsername = new DateTime($user['username_change_date']);
        $diff = $lastChangeUsername->diff($now);
...
function modifyChangeUsernameHistory($userid, $oldhandle, $newhandle, $date): void
{
    $history = json_decode(
    ...
    if (qa_clicked('dosaveprofile') && !$isblocked) {
        ...
        if (!qa_check_form_security_code('account', qa_post_text('code'))) {
            ...
            if (!isset($errors['handle'])) {
                if (null !== checkLastChangeUsername($userid) && checkLastChangeUsername($userid) < 30) {
