...
class qa_html_theme_layer extends qa_html_theme_base
{
    ...
    public function head_script()
    {
        ...
        if (qa_opt('q2apro_onsitenotifications_enabled')) {
            $this->output('<script type="text/javascript">var eventnotifyAjaxURL = "' . qa_path('eventnotify') . '";</script>');  
            ...
            if ('SnowFlat' == qa_opt('site_theme')) {
                $this->output('<script type="text/javascript">$(document).ready(function(){var elem = $("#osnbox").detach();$(".qam-account-items-wrapper").prepend(elem);});</script>');
            }
            
            if('Snow' === qa_opt('site_theme')) {
                $this->output('<style type="text/css">#nfyWrap {left:-100px;}</style>');
            }
    ...
    public function doctype()
    {
        ...
        if (qa_opt('q2apro_onsitenotifications_enabled') && $userId) {
            ...
            // first visit (I don't know why '81 D:, maybe unix timestamp start would be better?)
            if (is_null($lastVisit)) {
                $lastVisit = '1981-03-31 06:25:00';
            }
