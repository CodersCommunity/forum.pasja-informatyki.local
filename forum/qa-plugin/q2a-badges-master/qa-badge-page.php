	...
	class qa_badge_page {
		...
		function process_request($request)
		{
			...
			foreach($badges as $slug => $info) {
				...
				$types = $type['slug']; 
				$typen = $type['name']; 
				$qa_content['custom2'].='<tr><td class="badge-entry"><div class="badge-entry-badge"><span class="badge-'.$types.'" title="'.$typen.'">'.$name.'</span>&nbsp;<span class="badge-entry-desc">'.$desc.'</span>'.(isset($count[$slug])?'&nbsp;<span title="'.$count[$slug]['count'].' '.qa_lang('badges/awarded').'" class="badge-count-link" onclick="jQuery(\'#badge-users-'.$slug.'\').slideToggle()">x'.$count[$slug]['count'].'</span>':'').'</div>';
				...
				if(qa_opt('badge_show_source_users') && isset($count[$slug])) {
					...
					$qa_content['custom2'] .= implode(', ',$users).'</div>';
				}
