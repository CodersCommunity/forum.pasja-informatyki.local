...
*/
	...
	//	Created when and by whom
		...
		if (@$options['whoview']) {
			// get user level (only experts)
			$userlevels = qa_db_select_with_pending(qa_db_user_levels_selectspec($post['userid'], true, true));
			$levelincategory = 0;
			if (!empty($userlevels)) {
				foreach ($userlevels as $userlvl) {
					if ($post['categorybackpath'] == $userlvl['backpath']) {
						$levelincategory = $userlvl['level'];
					}
				}
			}
			if ($post['level'] > $levelincategory) {
				$levelincategory = $post['level'];
			}
			$fields['who']=qa_who_to_html($isbyuser, @$post['userid'], $usershtml, @$options['ipview'] ? @$post['createip'] : null, $microformats, $post['name'], $levelincategory);
	...
	//	Updated when and by whom
		...
		if (
			...
			if (isset($post['lastuserid']) && @$options['whoview']) {
				$lastuseraccountlevel = 0;
				if ($post['lastuserid'] > 0) {
					$lastuseraccount=qa_db_select_with_pending(qa_db_user_account_selectspec($post['lastuserid'], true));
					$lastuseraccountlevel=$lastuseraccount['level'];
					// only experts
					$userlevels = qa_db_select_with_pending(qa_db_user_levels_selectspec($post['lastuserid'], true, true));
					$levelincategory = 0;
					if (!empty($userlevels)) {
						foreach ($userlevels as $userlvl) {
							if ($post['categorybackpath'] == $userlvl['backpath']) {
					...
					if ($levelincategory > $lastuseraccountlevel) {
						$lastuseraccountlevel = $levelincategory;
					}
				}
				$fields['who_2']=qa_who_to_html(isset($userid) && ($post['lastuserid']==$userid), $post['lastuserid'], $usershtml, @$options['ipview'] ? $post['lastip'] : null, false, null, $lastuseraccountlevel);
			}
		}
...
*/
	...
	function qa_who_to_html($isbyuser, $postuserid, $usershtml, $ip=null, $microformats=false, $name=null, $userlevel=0)
/*
...
*/
	{
		...
		$toReturn = qa_lang_html_sub_split('main/by_x', $whohtml);

		// username colors
		if ($userlevel > 0) {
			switch ($userlevel) {
				case QA_USER_LEVEL_SUPER: $colorClass = 'administrator'; break;
				case QA_USER_LEVEL_ADMIN: $colorClass = 'administrator'; break;
			...
			if (!empty($colorClass)) {
				$toReturn['data'] = '<span class="level-'.$colorClass.'">'.$toReturn['data'].'</span>';
			}
		}
...
*/
	{
		...
		if (@$options['whoview']) {
			...
			// get user level (only experts)
			$userlevels = qa_db_select_with_pending(qa_db_user_levels_selectspec($question['ouserid'], true, true));
			$levelincategory = 0;
			if (!empty($userlevels)) {
				foreach ($userlevels as $userlvl) {
					if ($question['categorybackpath'] == $userlvl['backpath']) {
						$levelincategory = $userlvl['level'];
					}
				}
			}
			if ($question['olevel'] > $levelincategory) {
				$levelincategory = $question['olevel'];
			}
			$fields['who']=qa_who_to_html($isbyuser, $question['ouserid'], $usershtml, @$options['ipview'] ? @$question['oip'] : null, false, @$question['oname'], $levelincategory);
