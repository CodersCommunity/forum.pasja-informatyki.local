            ...
            elseif (qa_clicked('dosave')) {
                ...
                if (!qa_check_form_security_code('user-edit-'.$handle, qa_post_text('code'))) {
                    ...
                    if ($fieldseditable) {
                        ...
                        if ($loginlevel >= QA_USER_LEVEL_ADMIN && $inhandle != $useraccount['handle']) {

                            $handleusers=qa_db_user_find_by_handle($inhandle);
                    ...
                    if (empty($errors)) {
                        ...
                        if (!empty($inhandle) && $inhandle != $useraccount['handle']) {

                            qa_redirect('user/'.$inhandle);
...
//    General information about the user, only available if we're using internal user management
    ...
    if (!QA_FINAL_EXTERNAL_USERS) {
        ...
        $formshowlevel = qa_html(qa_user_level_string($useraccount['level']));

        if ($useraccount['level'] > 0) {
            ...
            if (!empty($colorClass)) {
                ...
                $formshowlevel = '<span class="level-'.$colorClass.'">'.$formshowlevel.'</span>';
    ...
    //    Private message link
        ...
        if ( $option && isset($loginuserid) && ($loginuserid != $userid) && !($useraccount['flags'] & QA_USER_FLAGS_NO_MESSAGES) && !$userediting ) {
			...
			$qa_content['form_profile']['fields']['level']['value'] .= strtr(qa_lang_html('profile/send_private_message'), array(
				...
				'^1' => '<br><dfn class="pw-link-admins" data-info="UĹĽytkownik ma wyĹ‚Ä…czone otrzymywanie wiadomoĹ›ci od innych uĹĽytkownikĂłw, ale korzystajÄ…c z uprawnieĹ„ administracyjnych moĹĽesz skontaktowaÄ‡ siÄ™ z nim"><a href="'.qa_path_html('message/'.$handle).'">',
        ...
        if ($userediting) {
            ...
            foreach ($userlevels as $userlevel) {
                ...
                if ( $userlevel['entitytype'] == QA_ENTITY_CATEGORY && $userlevel['level'] > $useraccount['level'] ) {
                    ...
                    $showcategorylevel = qa_html(qa_user_level_string($userlevel['level']));
                    ...
                    if ($userlevel['level'] > 0) {
                        ...
                        if (!empty($colorClass)) {
                            ...
                            $showcategorylevel = '<span class="level-'.$colorClass.'">'.$showcategorylevel.'</span>';
    ...
    //    Show any extra privileges due to user's level or their points
        ...
        if ($userediting && $loginlevel >= QA_USER_LEVEL_ADMIN && !qa_user_permit_error()) {

            $qa_content['form_profile']['fields']['handle'] = array(
